...
class DenoiseGANModel(BaseModel):
    def name(self):
        return 'DenoiseGANModel'

    def __init__(self, opt):
        super(DenoiseGANModel, self).__init__(opt)
        train_opt = opt['train']
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:  # feature loss
                real_fea = self.netF(self.var_H).detach() #don't need backward, so .detach()
                fake_fea = self.netF(self.fake_H)  #???? why don't need .detach()
