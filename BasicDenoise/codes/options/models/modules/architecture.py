...
class DenoiseResNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, upscale=4, norm_type='batch', act_type='relu', \
        ...
        stride = 2
        fea_conv = B.conv_block(in_nc, nf, kernel_size=3, stride=stride, norm_type=None, act_type=None)
        ...
        if upscale == 3:
            ...
            upscale_layer = int(math.log(upscale, 2))
            upsampler = [upsample_block(nf, nf, act_type=act_type) for _ in range(upscale_layer)]
...
class DnCNN(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, norm_type='batch', act_type='relu'):
        ...
        CNA_blocks = [B.conv_block(nf, nf, kernel_size=3, norm_type=norm_type, act_type=act_type) for _ in range(nb)]

        # this way is error !!
        # CNA = B.conv_block(nf, nf, kernel_size=3, norm_type=norm_type, act_type=act_type)
        # CNA_blocks = [CNA for _ in range(nb)]

        L_Conv = B.conv_block(nf, out_nc, kernel_size=3, norm_type=None, act_type=None)

        self.model = B.sequential(L_CA, *CNA_blocks, L_Conv)
...
class DnCNNSaoYan(nn.Module):
    def __init__(self, channels, num_of_layers=17):
        ...
        layers.append(nn.Conv2d(in_channels=channels, out_channels=features,
                                kernel_size=kernel_size, padding=padding, bias=False)) #bias=False
